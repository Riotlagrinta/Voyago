...
export default function BusesManagement() {
  ...
  const handleAddBus = async () => {
    try {
      ...
      console.error("Erreur ajout bus", err);
    }
  };
      ...
      <Card className="p-8 border-none shadow-voyago rounded-[2rem] bg-white overflow-hidden">
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          ...
          <div className="flex gap-2">
            <Badge variant="default" className="cursor-pointer hover:bg-surface">Tous (18)</Badge>
            <Badge variant="success" className="cursor-pointer hover:opacity-80">Actifs (15)</Badge>
            <Badge variant="warning" className="cursor-pointer hover:opacity-80">Maintenance (3)</Badge>
          </div>
        </div>
      ...
      <Modal 
        ...
        footer={
          <>
            ...
            <Button onClick={handleAddBus} className="px-8">Confirmer l'ajout</Button>
          </>
        }
      >
        <div className="space-y-6">
          ...
          <div className="p-4 bg-primary/5 border border-primary/10 rounded-2xl flex items-start gap-3">
            ...
            <p className="text-xs text-foreground/60 leading-relaxed">
              Le plan des sièges sera automatiquement généré en fonction de la capacité choisie (configuration standard 2x2).
            </p>
          </div>
        </div>
      </Modal>
